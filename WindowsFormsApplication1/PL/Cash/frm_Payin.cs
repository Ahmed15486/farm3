...
{
    public partial class frm_Payin : Form
    {
        #region Declarations
        public int UserID;
        BL.BL.PayIn payin = new BL.BL.PayIn();
        ...
        void var()
        {
            payin.ID = txt_ID.Text;
            payin.Date = DateTime.ParseExact(txt_Date.Text.Trim(), "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None);
            payin.No = txt_No.Text.Trim();
            payin.ACCID = com_ACC.SelectedValue.ToString();
            payin.Value = Convert.ToDecimal(txt_Value.Text.Trim());
            payin.PayTypeID = Convert.ToInt32(com_PayType.SelectedValue);
            payin.ChekNo = txt_ChekNo.Text;
            if(txt_CheckDate.Text.Trim() != "") payin.CheckDate = Convert.ToDateTime(txt_CheckDate.Text.Trim());
            payin.Notes = txt_Notes.Text.Trim();
            payin.UserID = UserID;
        }
        void Fill()
        {
            ...
            dgv.DataSource = null;
            dgv.DataSource = dt;
            //dgv.DataSource = g.Select2("Select * from  PayIn");
            //dgv.DataSource = g.Select2("Select * from  PayIn Where ACCID = '" + com_ACC.SelectedValue.ToString() + "'");
        }
        ...
        #region Form
        private void frm_Payin_Shown(object sender, EventArgs e)
        {
            if (dgv.Rows.Count > 0)
            {
                Form_Mode("Select");
