...
namespace WindowsFormsApplication1.PL.Store
{
    public partial class frm_OpenStock : Form
    {
        ...
        PL.Store.frm_OpenAdd add = new frm_OpenAdd();
        BL.BL.Units units = new BL.BL.Units();

        DataTable Temp_dgv = new DataTable();
        BL.BL.Open open = new BL.BL.Open();
        ...
        void var()
        {
            open.ID = (txt_ID.Text == "") ? 0 : Convert.ToInt32(txt_ID.Text);
            open.Date = DateTime.ParseExact(txt_Date.Text.Trim(), "dd/MM/yyyy", null);
            open.Notes = txt_Notes.Text;
            open.UserID = UserID;
            open.IO = table();
        }
        void EmptyZero()
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            if (Tag.ToString() == "New")
            {
                open.ID = Convert.ToInt32(open.Insert());
                open.nav = -1;
                Form_Mode("Select");
            }
            else if (Tag.ToString() == "Edit")
            {
                open.ID = Convert.ToInt32(open.Update());
                open.nav = -1;
                Form_Mode("Select");
            }
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
            ...
            {
                open.ID = Convert.ToInt32(txt_ID.Text);
                string t = open.Delete();
            }
            Form_Mode("Empty");
        }
        ...
        private void textBox_Search_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                open.ID = Convert.ToInt32(txt_Search.Text.Trim());
                open.nav = -1;
                Form_Mode("Select");
            }
        }
